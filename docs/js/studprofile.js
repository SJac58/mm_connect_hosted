...
function renderProfile(data) {
  const { student, performances, extracurricular, meetingsPast, meetingsUpcoming, notes } = data;
  ...
  meetingsUpcoming.forEach((m) => {
    const li = document.createElement("li");
    li.innerHTML = `<div class="meeting-time">${m.meeting_date} ${m.time_slot || ""}</div><div class="meeting-reason">${m.reason || ""}</div>`;
  ...
  meetingsPast.forEach((m) => {
    const li = document.createElement("li");
    li.innerHTML = `<div class="meeting-time">${m.meeting_date} ${m.time_slot || ""}</div><div class="meeting-reason">${m.reason || ""}</div><div class="muted small">Feedback: ${m.feedback || "—"}</div>`;
  ...
  if (performances && performances.length > 0) {
    ...
    els.marksRow.innerHTML = `<td>${latest.CIA1 ?? "-"}</td><td>${latest.CIA2 ?? "-"}</td><td>${latest.CIA3 ?? "-"}</td><td>${latest.ESE ?? "-"}</td>`;

    const labels = performances.map((p) => "Sem " + p.semester);
    const dataPoints = performances.map((p) => {
      const vals = [Number(p.CIA1 || 0), Number(p.CIA2 || 0), Number(p.CIA3 || 0), Number(p.ESE || 0)];
      return Math.round((vals.reduce((a, b) => a + b, 0) / (vals.length || 1)) * 100) / 100;
    });

    renderChart(labels, dataPoints);

    const overall = Math.round((dataPoints.reduce((a, b) => a + b, 0) / (dataPoints.length || 1)) * 100) / 100;
    els.overallProgress.textContent = (overall || "-") + "%";
    els.academicProgress.textContent = (dataPoints[dataPoints.length - 1] || "-") + "%";
...
function renderChart(labels, dataPoints) {
  ...
  chartInstance = new Chart(ctx, {
    type: "line",
    data: { labels, datasets: [{ label: "Average (%)", data: dataPoints, borderWidth: 2, tension: 0.3, fill: false }] },
...
function renderNotes(notes) {
  ...
  notes.forEach((n) => {
    ...
    div.innerHTML = `<div class="muted small">By Mentor ${n.mentor_id} · ${new Date(n.created_at).toLocaleString()}</div><div>${escapeHtml(n.note)}</div>`;
...
els.noteForm.addEventListener("submit", async (e) => {
  ...
  try {
    const res = await fetch("/api/notes", {
      ...
      body: JSON.stringify({ mentor_id: mentorId, student_reg_num: regNum, note }),
    });
    if (!res.ok) {
